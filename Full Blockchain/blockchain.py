...
class Blockchain:
    ...
    def add_block(self, block_data=[]):
        ...
        block_data.extend(self.txs)
        #print(self.txs)
        #print(self.blocks)

        block_dict = {'txs': block_data, 'h': {'ph': sha256(json.dumps(self.blocks[-1].header).encode()).digest().hex()}}
        ...
        for block in blocks:
            for transaction in block.data:
                if transaction['dest'] == pub_key:
                    running_total += int(transaction['val'].decode('utf-8'))
                if transaction['src'] == pub_key:
                    running_total -= int(transaction['val'].decode('utf-8'))
...
if __name__ == "__main__":
    ...
    #         pubk = prik.publickey()
    #
    #         h = SHA256.new(pubk.export_key('PEM'))
    #         dest = RSA.generate(1024).publickey()
    #         h.update(dest.export_key('PEM'))
    #         h.update(b'100')
    #         signer = pkcs1_15.new(PRIVATE_KEY).sign(h)
    #         blocky.add_tx({'src': LOCAL_ADDRESS, 'dest': dest.export_key('PEM'), 'val': b'100', 'sig': signer})
    #     blocky.add_block()
    # print(blocky.dump())
